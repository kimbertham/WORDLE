...
var friendResult = function (_a) {
    ...
    var _b = react_1.useState(false), showOpp = _b[0], setShowOpp = _b[1];
      ...
      <div className='rmid'>
        ...
        {!showOpp && <>
          ...
          <div className='winner'>
            <p>Winner</p>
            <h1>{lib_1.cap(getWinner())}</h1> 
          </div>

          <div className='resScores'>
            <div>
              <h2>{lib_1.cap(p1.user.username)}</h2>
              <p>Word: {p1.word}</p>
              <p>{p1.completed ?
                "Guesses:" + p1.guesses.length + "/6"
                : p1.user.username + " hasnt made their guesses yet"}</p>
            </div>

            <div>
              <h2>{lib_1.cap(p2.user.username)}</h2>
              <p>Word: {p2.word}</p>
              <p>{p2.completed ?
                "Guesses:" + p2.guesses.length + "/6"
                : p2.user.username + " hasnt made their guesses yet"}</p>
            </div>
          </div>
          ...
          {p1.completed && p2.completed &&
            ...
            <button onClick={function () { return history.push("/new/" + currentRound.friendship); }}> 
              <p>New Game </p>
            </button>
